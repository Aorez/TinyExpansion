...
/* uses linear search */
static TokenType2 reservedLookup (char * s)
{ int i;
 ...
 */
TokenType2 getToken(void)
{  /* index for storing into tokenString */
   int tokenStringIndex = 0;
   /* holds current token to be returned */
   TokenType2 currentToken;
   ...
   { int c = getNextChar();
     ...
     { case START:
         if (isdigit(c))
             ...
             state = (StateType)(int)ASSIGN;//冒号等号修改为正则表达式的赋值
         else if (c == '=')//修改为等号
             state = INASSIGN;
         else if (c == '-')//减号单独判断，因为加上了减法赋值号
             state = (StateType)(int)MINUS;
         else if (c == '>') {
             state = (StateType)(int)GT;//加入大于号
         }
         else if (c == '<') {
             state = (StateType)(int)LT;//小于号单独判断
         }
         else if ((c == ' ') || (c == '\t') || (c == '\n'))
           save = FALSE;
         else if (c == '{')
